...
class TMainEntity : public TCommonTable {
	...
	TADOQuery* Query;
	int SumOfMinutes;
	std::map<int,double> AverageLineSpeed; // int - productID, double - AverageLineSpeed
...
public:
    ...
    UnicodeString GetDOCNUM() { return IdGIP; }
	void Remove(int) {
	};
	double GetAverageLineSpeed(int ProdID) { return AverageLineSpeed[ProdID]; }
	void SetAverageLineSpeed(int ProdID,double Speed) { AverageLineSpeed[ProdID] = Speed; };
};
