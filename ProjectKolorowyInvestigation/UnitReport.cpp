...
std::map<char*,int> SunGuardsTimeEvent::ProdTypeFromAdditional;
std::map<int, char*> SunGuardsTimeEvent::ProdFullNameFromProdType;
...
TDateTime ShiftStartTime()
{
	//TDateTimePicker* DTP =  FormMain->DateTimePickerStartTime;
	TDateTime D = FormMain->DateTimePickerStartTime->DateTime ;
	switch(Current.GetShiftKind())
	{
		case 1: D.Val = int(D.Val)+0.25;
				break;
		case 2: D.Val =int(D.Val) + 14.0/24;
				break;
		case 3: D.Val = int(D.Val) + 22.0/24;
				break;

	}
	   //TDateTimePicker* DTP_T = FormMain->DateTimePickerStartTime;
...
TDateTime ShiftStopTime()
{
   TDateTimePicker* DTP =  FormMain->DateTimePickerMain;
	switch(Current.GetShiftKind())
	{
		case 1: DTP->Time =14.0/24;
				break;
		case 2: DTP->Time = 22.0/24;
				break;
		case 3: DTP->Time = 0.25 ;
				break;

	}
	   //TDateTimePicker* DTP_T = FormMain->DateTimePickerStartTime;

	return DTP->DateTime;	return DTP->DateTime;
}
...
std::pair<TDateTime,TDateTime> SetDateShiftWise2(TDateTime Begin, TDateTime End) // Set local for this file variables Start, Stop shiftwise  .
//reporting is shiftwise therfore we need to change start and stop date.
{
	  unsigned short year;
	  unsigned short month;
...
//	   }
//	   Stop = TDateTime(TDateTime(Stop).DateString()+" "+TDateTime(23,59,59,0).TimeString()); //reporting is shiftwise therfore we need to change start and stop date.
	   std::pair<TDateTime,TDateTime> Dates;
	   Dates.first = Start;
	   Dates.second  = Stop;
	   return Dates;
}
...
try {
	   ...
	   "		                AND DEFECTID <> 14                                                                   "   // 14 - Development
...
double TargetInput(double Speed)
{
		 ...
		 Cycle = (6.1/Speed)*60;
		 Target  =(3600.0/Cycle)*6*3.21;
		 return Target;
}

TOEE_Struct NewOEE(int ShiftID=0)
{
	  try{
...
//		  Query->Open();
//		  Query->ParamCheck=true;
//		  PrevID = Query->FieldByName("PreviousID")->AsInteger;
//	   }
//	   else
//	   {
//		 PrevID=0;
...
//		 Query->SQL->Add(QueryString);
//		 Query->Parameters->ParamByName("ShiftID")->Value = ShiftID;
		 //SetDateShiftWise(ShiftStartTime(),ShiftStopTime());
		 //Query->Parameters->ParamByName("PrevID")->Value = PrevID;
		 //AvailableTime = 480;
	   ...
	   double SetupRejects  = double(KPIReport.SetupByProd.Summary());
	   if (SetupRejects == 0)
	   {
            SetupRejects = 1;
	   }
	   OEE_Struct.ProductionTime = KPIReport.Uptime.Uptime();  //Schedule.GetNoMin(1);  // 1 - means Uptime
	   int StandByTime =  KPIReport.ScheduledDowntime.StandBy; //Schedule.GetNoMin(8);    // 8 - means StandBy
	   StandByTime += KPIReport.ScheduledDowntime.Development;
	   OverallTime = KPIReport.WholeTime;
	   AvailableTime = OverallTime - StandByTime;
	   double UptimeHour = OEE_Struct.ProductionTime/60.0;
	   OEE_Struct.Start=Start;
	   OEE_Struct.Stop=Stop;

	   if(AvailableTime!=0)
	   {
				OEE_Struct.OperationalUptime = OEE_Struct.ProductionTime / double(AvailableTime);
				OEE_Struct.Uptime = OEE_Struct.ProductionTime/ double(OverallTime);
	   }
	   else
	   {
				OEE_Struct.Uptime = 0;
				OEE_Struct.OperationalUptime = 0;
	   }
	   //TShiftProd Production;
	   //ProductionSummary(Production, ShiftID);
	   //Schedule.ReportPrepare();
	   double In = KPIReport.Input.Input();
	   double Out = KPIReport.Output.Output();
	   if (In !=0) {
			 OEE_Struct.Yield = Out/(In + KPIReport.Reject.Development);
			 OEE_Struct.OperationalYield = Out/(In );
	   }
	   else
	   {
			OEE_Struct.Yield =0;
			OEE_Struct.OperationalYield = 0;
	   }
	   //OEE_Struct.Speed = 11; // theoretical maximum line speed for ClimaGuard;
	   //double MaxInputPerHour = 2084;
	   ;
	   double Uptime;
	   double InputPerProdCategory[4];
	   Uptime = KPIReport.Uptime.CGA;
	   Uptime = Uptime / 60;
	   if (Uptime==0) {
		 OEE_Struct.NewPerformanceByProduct[0]=0;
	   }
	   else
	   {
		 InputPerProdCategory[0]= KPIReport.Input.CGA - KPIReport.Reject.QCBurnIns*KPIReport.SetupByProd.CGA/SetupRejects;
		 InputPerProdCategory[0]= InputPerProdCategory[0]/(KPIReport.Uptime.CGA/60.0);
		 OEE_Struct.NewPerformanceByProduct[0]=InputPerProdCategory[0]/TargetInput(OEE_Struct.NewMaxLineSpeedByProduct[0]);
		 //OEE_Struct.NewPerformanceByProduct[0]=(KPIReport.Input.CGA/KPIReport.Uptime.CGA)/TargetInput(OEE_Struct.NewMaxLineSpeedByProduct[0]);

	   }
	   Uptime = KPIReport.Uptime.CGHT;
		Uptime = Uptime / 60;
	   if (Uptime==0) {
		 OEE_Struct.NewPerformanceByProduct[1]=0;
	   }
	   else
	   {
		 InputPerProdCategory[1]= KPIReport.Input.CGHT - KPIReport.Reject.QCBurnIns*KPIReport.SetupByProd.CGHT/SetupRejects;
		 InputPerProdCategory[1]= InputPerProdCategory[1]/(KPIReport.Uptime.CGHT/60.0);
		 OEE_Struct.NewPerformanceByProduct[1]=InputPerProdCategory[1]/TargetInput(OEE_Struct.NewMaxLineSpeedByProduct[1]);
	   //OEE_Struct.NewPerformanceByProduct[1]=(KPIReport.Input.CGHT/KPIReport.Uptime.CGHT)/TargetInput(OEE_Struct.NewMaxLineSpeedByProduct[1]);
	   }
	   Uptime = KPIReport.Uptime.SGA;
		Uptime = Uptime / 60;
	   if (Uptime==0) {
		 OEE_Struct.NewPerformanceByProduct[2]=0;
	   }
	   else
	   {
		 InputPerProdCategory[2]= KPIReport.Input.SGA - KPIReport.Reject.QCBurnIns*KPIReport.SetupByProd.SGA/SetupRejects;
		 InputPerProdCategory[2]= InputPerProdCategory[2]/(KPIReport.Uptime.SGA/60.0);
		 OEE_Struct.NewPerformanceByProduct[2]=InputPerProdCategory[2]/TargetInput(OEE_Struct.NewMaxLineSpeedByProduct[2]);
		 //OEE_Struct.NewPerformanceByProduct[2]=(KPIReport.Input.SGA/KPIReport.Uptime.SGA)/TargetInput(OEE_Struct.NewMaxLineSpeedByProduct[2]);
	   }
	   Uptime = KPIReport.Uptime.SGHT;
		Uptime = Uptime / 60;
	   double TempDouble;
	   if (Uptime==0) {
		 OEE_Struct.NewPerformanceByProduct[3]=0;
	   }
	   else
	   {
		 InputPerProdCategory[3]= KPIReport.Input.SGHT - KPIReport.Reject.QCBurnIns*KPIReport.SetupByProd.SGHT/SetupRejects;
		 InputPerProdCategory[3]= InputPerProdCategory[3]/(KPIReport.Uptime.SGHT/60.0);
		 OEE_Struct.NewPerformanceByProduct[3]=InputPerProdCategory[3]/TargetInput(OEE_Struct.NewMaxLineSpeedByProduct[3]);
		 TempDouble = OEE_Struct.NewPerformanceByProduct[3];
		// OEE_Struct.NewPerformanceByProduct[3]=(KPIReport.Input.SGHT/KPIReport.Uptime.SGHT)/TargetInput(OEE_Struct.NewMaxLineSpeedByProduct[3]);
	   }

	   double ProdPerformance = 0;
	   ProdPerformance +=  OEE_Struct.NewPerformanceByProduct[0]*KPIReport.Uptime.CGA;
	   ProdPerformance +=  OEE_Struct.NewPerformanceByProduct[1]*KPIReport.Uptime.CGHT;
	   ProdPerformance +=  OEE_Struct.NewPerformanceByProduct[2]*KPIReport.Uptime.SGA;
	   ProdPerformance +=  OEE_Struct.NewPerformanceByProduct[3]*KPIReport.Uptime.SGHT;

	   if (OEE_Struct.ProductionTime!=0) {
		 OEE_Struct.NewPerformance = ProdPerformance/(OEE_Struct.ProductionTime);
	   }
	   else
	   {
		  OEE_Struct.NewPerformance = 0;
	   }

	   double TheoreticalCGInput = OEE_Struct.MaxInputPerHour*UptimeHour;//3.31*AvailableTime*Speed*(6/6.1);
	   if (TheoreticalCGInput!=0) {
		  OEE_Struct.Performance = ( KPIReport.InputConvertedRaw() )/TheoreticalCGInput;
	   }
	   else
	   {
		   OEE_Struct.Performance = 0;
	   }
	   OEE_Struct.NewOEE = OEE_Struct.OperationalYield*OEE_Struct.OperationalUptime*OEE_Struct.NewPerformance*100;
	   OEE_Struct.OEE = OEE_Struct.Yield*OEE_Struct.OperationalUptime*OEE_Struct.Performance*100;
	   return OEE_Struct;
	}
	__finally{
	   DataModule1->ADOConnectionCreport->Connected=false;
...
//	   Stop = TDateTime(TDateTime(Stop).DateString()+" "+TDateTime(23,59,59,0).TimeString()); //reporting is shiftwise therfore we need to change start and stop date.
	  ...
	  // SetDateShiftWise();
	   ...
	   Query->Close();
	   OEE_Struct.ProductionTime =  Schedule.GetNoMin(1);  // 1 - means Uptime
	   int StandByTime =  Schedule.GetNoMin(8);    // 8 - means StandBy
	   OverallTime = AvailableTime;
	   AvailableTime -= StandByTime;
	   double UptimeHour = OEE_Struct.ProductionTime/60;
	   OEE_Struct.Start=Start;
	   OEE_Struct.Stop=Stop;

	   if(AvailableTime!=0)
	   {
				OEE_Struct.OperationalUptime = double(OEE_Struct.ProductionTime) / AvailableTime;
				OEE_Struct.Uptime = double(OEE_Struct.ProductionTime)/ OverallTime;
	   }
	   else
	   {
				OEE_Struct.Uptime = 0;
   ...
   double ProductionInputConverted = KPIReport.InputConverted.CGA + KPIReport.InputConverted.CGHT +KPIReport.InputConverted.SGA + KPIReport.InputConverted.SGHT;
	   if (TheoreticalCGInput!=0) {
		  OEE_Struct.Performance = double(ProductionInputConverted)/TheoreticalCGInput;
	   }
	   else
	   {
		   OEE_Struct.Performance = 0;
...
void SunGuardsTimeGenerate()
// function fulfils  variable Schedule of SunGuardsTimeClass type
{
	try{
	   DataModule1->ADOConnectionCreport->Connected=true;
	   if(DataModule1->ADOConnectionCreport->Connected==false)
	  ...
	  for(it=DefectCategoryName.begin();it!=DefectCategoryName.end();it++,i++)
	  {
		 ...
		 switch (DefectCatID) {
			   ...
			   case 5: KPIReport.Reject.QCBurnIns  =CategoryAreas[DefectCatID];
									   break;
			   case 6: KPIReport.Reject.ContinuousImprovement=CategoryAreas[DefectCatID];
									   break;
		 default:
		  ;
		 }
...
void ProductInfoKPI()
{
   ...
   "		ORDER BY ID                                                                                                                                                                                                            ";
	   ...
	   KPIReport.ProductionMix.Commercial =0;
	   KPIReport.ProductionMix.Residential =0;
	   if (SummaryOutput!=0) {
...
void SunGuardsTimeEvent::SetValues(TDateTime Date, char Shift, TDateTime StartT, TDateTime EndT,UnicodeString Full, UnicodeString Short,UnicodeString LineStat,UnicodeString Remark, int Prod,int LineStatID, UnicodeString Additional)
{
	DateOfWork = Date;
	ShiftC = Shift;
	StartTime = StartT;
	...
	if (LineStatID==4) // 4 - means change of product, then product is equal to Additional and not Prod
	{
			ProdType = GetProdIDFromAdd(Additional);
	}
	else
	{
		ProdType = Prod;
	...
	void SunGuardsTimeEvent::ProdTypeInitialize()
	{
	  try{
	   if (SunGuardsTimeEvent::ProdTypeFromAdditional.size()>0) {
		  return;
	   }
	   DataModule1->ADOConnectionReport2->Connected=true;
	   if(DataModule1->ADOConnectionReport2->Connected==false)
	   {
		   ShowMessage("Nie uda³o siê po³aczyc z baz¹ Creport, \nprawdopodobnie brak uprawnieñ");
		   return;
	   }
	   TADOQuery* Query = DataModule1->ADOQueryInitializeAdditionalToProdID;     //ADOQueryLineStatusReport;
	   AnsiString QueryString;
	   Query->Close();
	   Query->SQL->Clear();
	   QueryString=	" SELECT  [ID_PRODTYPE], [PRODTYPE_NAME],[FULLNAME]  "
					" FROM [ProdExtended] "
				;
	   ...
	   Query->Open();
	   SunGuardsTimeEvent::ProdTypeFromAdditional.clear();
	   int NoRec = Query->RecordCount;

	   for(int i=1;i<=NoRec;i++)               //  , round(sum(Rozm),0) Rozmiar
	   {
		   SunGuardsTimeEvent::ProdTypeFromAdditional[Query->FieldByName("PRODTYPE_NAME")->AsString]=Query->FieldByName("ID_PRODTYPE")->AsInteger;
		   SunGuardsTimeEvent::ProdFullNameFromProdType[Query->FieldByName("ID_PRODTYPE")->AsInteger]=Query->FieldByName("FULLNAME")->AsString;
		   Query->Next();
	   }
	   Query->Close();
	}
	__finally{
	   DataModule1->ADOConnectionReport2->Connected=false;
	}

	}

	int SunGuardsTimeEvent::GetProdIDFromAdd(UnicodeString Additional)
	{
	  try{
		 int ID;
		 ID= SunGuardsTimeEvent::ProdTypeFromAdditional[Additional];
		 return ID;
	  }
	  catch(...)
	  {
		ShowMessage("There were unexpected error in function SunGuardsTimeEvent::SetGetProdID \nWystąpił błąd w/w funckcji");
		return -1;
	  }
    }
...
void IsGreater()
{
	...
	LeftEvent.SetValues(TDateTime().CurrentDate(),'D',Left->Time,Right->Time,L"Coœ",L"Coœ jeszcze",L"Awaria",L"Remark",1,0,"0" );
	RightEvent.SetValues(TDateTime().CurrentDate(),'c',Right->Time,Right->Time,L"Coœ",L"Coœ jeszcze",L"Awaria",L"Remark",1,0,"0" );
...
void SunGuardsTimeClass::Add(TADOQuery* Q)
{
		  ...
		  Event.SetValues(DateOfWork,Shift[1],Start,Stop,Product,Short,LineStatus,Remarks,ProdID,LineStID,Additional);
	...
	void SunGuardsTimeClass::ReportPrepare()
	 //Fulfils KPIReport object (mainly it consists of various structures)
	{
		list<SunGuardsTimeEvent>::iterator it;
...
//					   }
			   ...
			   case 2:  Service = true;   //Service
						...
						break;
			   case 6:  //Development
						KPIReport.ScheduledDowntime.Development += ElapsTime;
						break;

			   case	39:  //Continuous Improvement
						 KPIReport.ScheduledDowntime.ContinuousImprovement += ElapsTime;
							break;
			   ...
			   case 5:					//Pumping conditioning
				...
				case  4 :		//Setup/Burn-in
							...
							{
									  case 6:		        //ClimaGuardsA
										   KPIReport.SetupByProd.CGA += ElapsTime;

													 break;
									  case  5:					//ClimaGuardsHT
											KPIReport.SetupByProd.CGHT += ElapsTime;
													 break;
									 case 4:			//SunGuardsA
											KPIReport.SetupByProd.SGA += ElapsTime;
													 break;
									  case 3:			//SunGuardsHT
											KPIReport.SetupByProd.SGHT += ElapsTime;
													 break;
									default:
									 ;
							}
...
//	int SunGuardsTimeClass::GetNoMin(int LineStatusID, std::set<int> )
//	{
//		  list<SunGuardsTimeEvent>::iterator it;
//		  int ID,i,SumMin;
//		  //TFont* Font;
...
//	ScheduledDowntime.Development=0;
...
//	ProductionMix.Residential=0;
 ...
 void TKPIReport::Clear()
 {
	...
	ScheduledDowntime.Setup=0;
	ScheduledDowntime.Development=0;
	ScheduledDowntime.ContinuousImprovement=0;
	ScheduledDowntime.MachineUpgrades=0;
	ScheduledDowntime.StandBy=0;
	...
	Reject.Equipment=0;
	Reject.QCBurnIns=0;
	Reject.ContinuousImprovement=0;
	Reject.Development=0;
 ...
 void TKPIReport::Prepare()
 {
   Clear();
   SunGuardsTimeGenerate();
   Schedule.ReportPrepare();
   BedUtilizationKPI();
   ProductInfoKPI();
   ShowDefectsKPI();
   double ProductionInput = KPIReport.Input.CGA + KPIReport.Input.CGHT +KPIReport.Input.SGA + KPIReport.Input.SGHT;
   double ProductionOutput = KPIReport.Output.CGA + KPIReport.Output.CGHT +KPIReport.Output.SGA + KPIReport.Output.SGHT;
   double ProductionInputConverted;
   if (ProductionInput==0) {
	 KPIReport.InputConverted.SGA =0   ;
	 KPIReport.InputConverted.SGHT =0  ;
	 KPIReport.InputConverted.CGA =0   ;
	 KPIReport.InputConverted.CGHT =0  ;
	 ProductionInputConverted =0       ;
   }
   else
   {
	 KPIReport.InputConverted.SGA =   (KPIReport.Input.SGA/ProductionInput)*KPIReport.Reject.Development + KPIReport.Input.SGA;
	 ...
	 KPIReport.InputConverted.CGA =   (KPIReport.Input.CGA/ProductionInput)*KPIReport.Reject.Development + KPIReport.Input.CGA;
	 KPIReport.InputConverted.CGHT =   (KPIReport.Input.CGHT/ProductionInput)*KPIReport.Reject.Development + KPIReport.Input.CGHT;
	 ProductionInputConverted = KPIReport.InputConverted.CGA + KPIReport.InputConverted.CGHT +KPIReport.InputConverted.SGA + KPIReport.InputConverted.SGHT;
   }

 }

 void TKPIReport::PrepareOLD()
 {
   Clear();
   SunGuardsTimeGenerate();
   Schedule.ReportPrepare();
 ...
 void TKPIReport::Generate()
 {
   ...
   double TempDouble;
		  ...
		  TempAnsi = TempAnsi.FormatFloat("0.00",ResultsOEE.NewOEE);
		  Sheet->Cells[1][Row ] = TempAnsi;
		  TempAnsi = "OEE (%): ";
		  Sheet->Cells[0][Row ] = TempAnsi;
		   Row++;

		  TempAnsi = TempAnsi.FormatFloat("0.00",ResultsOEE.OperationalUptime*100);
		  Sheet->Cells[1][Row ] = TempAnsi;
				  TempAnsi = "Operational Uptime OEE (%): ";
		  Sheet->Cells[0][Row ] = TempAnsi;
		  Row++;
		  ...
		  TempAnsi = TempAnsi.FormatFloat("0.00",ResultsOEE.NewPerformance*100);
		  Sheet->Cells[1][Row ] = TempAnsi;
		  ...
		  FieldWidth[1]=Width>FieldWidth[1]?Width:FieldWidth[1];
		  Sheet->Cells[1][Row ] = TempAnsi;
		  TempAnsi = "Scheduled Downtime with StandBy and Development (h) ";
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[0]=Width>FieldWidth[0]?Width:FieldWidth[0];
		  Sheet->Cells[0][Row ] = TempAnsi;
		  ...
		  FieldWidth[4]=Width>FieldWidth[4]?Width:FieldWidth[4];
		  Sheet->Cells[4][Row ] = TempAnsi;
		  TempAnsi = "Scheduled Downtime with StandBy and Development (%) ";
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[3]=Width>FieldWidth[3]?Width:FieldWidth[3];
		  Sheet->Cells[3][Row ] = TempAnsi;
		  Row++;
		  ...
		  FieldWidth[1]=Width>FieldWidth[1]?Width:FieldWidth[1];
		  Sheet->Cells[1][Row ] = TempAnsi;
		  TempAnsi = "Operational Scheduled Downtime (without StandBy and Development)  (h) ";
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[0]=Width>FieldWidth[0]?Width:FieldWidth[0];
		  Sheet->Cells[0][Row ] = TempAnsi;

		  TempAnsi = TempAnsi.FormatFloat("0.00",(ScheduledDowntime.ScheduledOperationalDowntime()  /double(OperationalTime()))*100.0) ;
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[4]=Width>FieldWidth[4]?Width:FieldWidth[4];
		  Sheet->Cells[4][Row ] = TempAnsi;
...
//		  Sheet->Cells[0][Row ] = TempAnsi;
		  ...
		  FieldWidth[4]=Width>FieldWidth[4]?Width:FieldWidth[4];
		  Sheet->Cells[4][Row ] = TempAnsi;
		  TempAnsi = "Operational Scheduled Downtime\n(Scheduled - StandBy-Development)/(Month hours)  (%)";
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[3]=Width>FieldWidth[3]?Width:FieldWidth[3];
		  Sheet->Cells[3][Row ] = TempAnsi;
		  ...
		  FieldWidth[0]=Width>FieldWidth[0]?Width:FieldWidth[0];
		  Sheet->Cells[0][Row ] = TempAnsi;
		  TempAnsi = TempAnsi.FormatFloat("0.00",(ScheduledDowntime.Overhaul.Vent/double(OperationalTime()))*100.0) ;
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[4]=Width>FieldWidth[4]?Width:FieldWidth[4];
		  Sheet->Cells[4][Row ] = TempAnsi;
		  ...
		  TempAnsi = TempAnsi.FormatFloat("0.00",(ScheduledDowntime.Overhaul.Pumping /double(OperationalTime()))*100.0) ;
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[4]=Width>FieldWidth[4]?Width:FieldWidth[4];
		  Sheet->Cells[4][Row ] = TempAnsi;
		  ...
		  TempAnsi = TempAnsi.FormatFloat("0.00",(ScheduledDowntime.Setup /double(OperationalTime()))*100.0) ;
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[4]=Width>FieldWidth[4]?Width:FieldWidth[4];
		  Sheet->Cells[4][Row ] = TempAnsi;
		  TempAnsi = "Setup (%) ";
		  ...
		  TempAnsi = TempAnsi.FormatFloat("0.00", ScheduledDowntime.Development /60.0) ;
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[1]=Width>FieldWidth[1]?Width:FieldWidth[1];
		  Sheet->Cells[1][ Row] = TempAnsi;
		  ...
		  TempAnsi = TempAnsi.FormatFloat("0.00",(ScheduledDowntime.Development /double(OperationalTime()))*100.0) ;
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[4]=Width>FieldWidth[4]?Width:FieldWidth[4];
		  Sheet->Cells[4][Row ] = TempAnsi;
		  ...
		  TempAnsi = TempAnsi.FormatFloat("0.00",(ScheduledDowntime.MachineUpgrades /double(OperationalTime()))*100.0) ;
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[4]=Width>FieldWidth[4]?Width:FieldWidth[4];
		  Sheet->Cells[4][Row ] = TempAnsi;
		  ...
		  TempAnsi = TempAnsi.FormatFloat("0.00",(ScheduledDowntime.ContinuousImprovement)/60.0) ;
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[1]=Width>FieldWidth[1]?Width:FieldWidth[1];
		  Sheet->Cells[1][Row ] = TempAnsi;
		  TempAnsi = "Continuous Improvement (h)  ";
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[0]=Width>FieldWidth[0]?Width:FieldWidth[0];
		  Sheet->Cells[0][Row ] = TempAnsi;

		  TempAnsi = TempAnsi.FormatFloat("0.00",(ScheduledDowntime.ContinuousImprovement /double(OperationalTime()))*100.0) ;
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[4]=Width>FieldWidth[4]?Width:FieldWidth[4];
		  Sheet->Cells[4][Row ] = TempAnsi;
		  TempAnsi = "Continuous Improvement (%) ";
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[3]=Width>FieldWidth[3]?Width:FieldWidth[3];
		  Sheet->Cells[3][Row ] = TempAnsi;
		  ...
		  Sheet->Cells[3][Row ] = TempAnsi;
		  Row++;
		  TempAnsi = TempAnsi.FormatFloat("0.00",(UnscheduledDowntime.UnscheduledDowntime() /OperationalTime())*100.0) ;
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[4]=Width>FieldWidth[4]?Width:FieldWidth[4];
		  Sheet->Cells[4][Row ] = TempAnsi;
		  ...
		  TempAnsi = TempAnsi.FormatFloat("0.00",(UnscheduledDowntime.ProcessIssues  /OperationalTime())*100.0) ;
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[4]=Width>FieldWidth[4]?Width:FieldWidth[4];
		  Sheet->Cells[4][Row ] = TempAnsi;
		  ...
		  TempAnsi = TempAnsi.FormatFloat("0.00",(UnscheduledDowntime.AncillaryEquipment  /OperationalTime())*100.0) ;
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[4]=Width>FieldWidth[4]?Width:FieldWidth[4];
		  Sheet->Cells[4][Row ] = TempAnsi;
		  ...
		  TempAnsi = TempAnsi.FormatFloat("0.00",(UnscheduledDowntime.Coater  /OperationalTime())*100.0) ;
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[4]=Width>FieldWidth[4]?Width:FieldWidth[4];
		  Sheet->Cells[4][Row ] = TempAnsi;
		  TempAnsi = "Coater Equipment Downtime (%) ";
		  ...
		  TempAnsi = TempAnsi.FormatFloat("0.00",(UnscheduledDowntime.ProductionOrganization  /OperationalTime())*100.0) ;
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[4]=Width>FieldWidth[4]?Width:FieldWidth[4];
		  Sheet->Cells[4][Row ] = TempAnsi;
		  ...
		  TempAnsi = TempAnsi.FormatFloat("0.00",(UnscheduledDowntime.WasherAndWatertreatment  /OperationalTime())*100.0) ;
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[4]=Width>FieldWidth[4]?Width:FieldWidth[4];
		  Sheet->Cells[4][Row ] = TempAnsi;
		  ...
		  Row++;
		  TempAnsi = TempAnsi.FormatFloat("0.00",Reject.ContinuousImprovement)  ;
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[1]=Width>FieldWidth[1]?Width:FieldWidth[1];
		  Sheet->Cells[1][ Row] = TempAnsi;
		  TempAnsi = "Continuous Improvement Rejects (m^2)";
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[0]=Width>FieldWidth[0]?Width:FieldWidth[0];
		  Sheet->Cells[0][Row] = TempAnsi;

		  TempAnsi = TempAnsi.FormatFloat("0.00",(Reject.ContinuousImprovement  /double(Reject.Reject()))*100.0) ;
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[4]=Width>FieldWidth[4]?Width:FieldWidth[4];
		  Sheet->Cells[4][Row ] = TempAnsi;
		  TempAnsi = "Continuous Improvement Rejects (%) ";
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[3]=Width>FieldWidth[3]?Width:FieldWidth[3];
		  Sheet->Cells[3][Row ] = TempAnsi;
		  Row++;
		  ...
		  Sheet->Cells[3][Row ] = TempAnsi;
		  Row++;
		  TempAnsi = TempAnsi.FormatFloat("0.00",(Uptime.Uptime()/double(WholeTime-ScheduledDowntime.StandBy-ScheduledDowntime.Development))*100.0) ;
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[4]=Width>FieldWidth[4]?Width:FieldWidth[4];
		  Sheet->Cells[4][Row ] = TempAnsi;
		  ...
		  TempAnsi = TempAnsi.FormatFloat("0.00",InputConvertedRaw())   ;
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[1]=Width>FieldWidth[1]?Width:FieldWidth[1];
		  Sheet->Cells[1][Row ] = TempAnsi;
		  ...
		  TempAnsi = TempAnsi.FormatFloat("0.00",OperationalTime()/60.0);
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[4]=Width>FieldWidth[4]?Width:FieldWidth[4];
		  Sheet->Cells[4][Row ] = TempAnsi;
		  TempAnsi = "Total Available Time (Total-Standby-Development):";
		  Width =  Sheet->Canvas->TextWidth(TempAnsi);
		  FieldWidth[3]=Width>FieldWidth[3]?Width:FieldWidth[3];
		  Sheet->Cells[3][Row ] = TempAnsi;
